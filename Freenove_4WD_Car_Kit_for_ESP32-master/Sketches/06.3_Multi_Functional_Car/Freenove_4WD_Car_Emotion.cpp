#include <Arduino.h>
#include "Freenove_VK16K33_Lib_For_ESP32.h"
#include "Freenove_4WD_Car_Emotion.h"

//Turn eyes clockwise
byte eyeRotate1[][8] = {
  0x00, 0x00, 0x5C, 0xAA, 0x62, 0x1C, 0x00, 0x00, // custom dizzy x
  0x00, 0x00, 0x5C, 0xB2, 0x62, 0x1C, 0x00, 0x00,
  0x00, 0x00, 0x5C, 0xA2, 0x72, 0x1C, 0x00, 0x00,
  0x00, 0x00, 0x5C, 0xA2, 0x6A, 0x1C, 0x00, 0x00,
  0x00, 0x00, 0x5C, 0xA2, 0x66, 0x1C, 0x00, 0x00,
  0x00, 0x00, 0x5C, 0xA6, 0x62, 0x1C, 0x00, 0x00,
};

//Turn eyes counterclockwise
byte eyeRotate2[][8] = {
  0x00, 0x00, 0x3A, 0x55, 0x46, 0x38, 0x00, 0x00, // custom dizzy y
  0x00, 0x00, 0x3A, 0x65, 0x46, 0x38, 0x00, 0x00,
  0x00, 0x00, 0x3A, 0x45, 0x66, 0x38, 0x00, 0x00,
  0x00, 0x00, 0x3A, 0x45, 0x56, 0x38, 0x00, 0x00,
  0x00, 0x00, 0x3A, 0x45, 0x4E, 0x38, 0x00, 0x00,
  0x00, 0x00, 0x3A, 0x4D, 0x46, 0x38, 0x00, 0x00,
};

//Blink eyes
byte eyeBlink1[][8] = {
  0x00, 0x00, 0x5C, 0xAA, 0x62, 0x1C, 0x00, 0x00, // sleep eyes x
  0x00, 0x00, 0x00, 0x5C, 0xA2, 0x62, 0x00, 0x00, 
};

//Blink eyes
byte eyeBlink2[][8] = {
  0x00, 0x00, 0x3A, 0x55, 0x46, 0x38, 0x00, 0x00, // sleep eyes y
  0x00, 0x00, 0x00, 0x3A, 0x45, 0x46, 0x00, 0x00,
};

//keep smile
byte eyeSmile1[][8] = {
  0x00, 0x00, 0x5C, 0xA2, 0x62, 0x00, 0x00, 0x00, // custom happy face x
  0x00, 0x00, 0x00, 0x5C, 0xA2, 0x62, 0x00, 0x00,
};

//keep smile
byte eyeSmile2[][8] = {
  0x00, 0x00, 0x3A, 0x45, 0x46, 0x00, 0x00, 0x00, // custom happy face y
  0x00, 0x00, 0x00, 0x3A, 0x45, 0x46, 0x00, 0x00,
};

//cry-left
byte eyeCry1[][8] = {
  0x00, 0x00, 0x22, 0x62, 0x5C, 0x00, 0x00, 0x00,
  0x00, 0x00, 0x22, 0x62, 0x5C, 0x40, 0x00, 0x00,
  0x00, 0x00, 0x22, 0x62, 0x1C, 0x40, 0x40, 0x00, 
  0x00, 0x00, 0x22, 0x62, 0x1C, 0x00, 0x40, 0x40, 
  0x00, 0x00, 0x22, 0x62, 0x1C, 0x00, 0x00, 0x40, 
  0x00, 0x00, 0x22, 0x62, 0x1C, 0x00, 0x00, 0x00, 
  0x00, 0x00, 0x22, 0x62, 0x1C, 0x00, 0x00, 0x00, // custom cry x

};

//cry-right
byte eyeCry2[][8] = {
  0x00, 0x00, 0x44, 0x46, 0x38, 0x00, 0x00, 0x00, 
  0x00, 0x00, 0x44, 0x46, 0x3A, 0x00, 0x00, 0x00, 
  0x00, 0x00, 0x44, 0x46, 0x3A, 0x02, 0x00, 0x00, 
  0x00, 0x00, 0x44, 0x46, 0x38, 0x02, 0x02, 0x00, 
  0x00, 0x00, 0x44, 0x46, 0x38, 0x00, 0x02, 0x02, 
  0x00, 0x00, 0x44, 0x46, 0x38, 0x00, 0x00, 0x02, 
  0x00, 0x00, 0x44, 0x46, 0x38, 0x00, 0x00, 0x00, // custom cry y

};

//wheel-left
byte wheel_left1[][8] =
{
  0x00, 0x02, 0x59, 0xA4, 0x62, 0x1C, 0x00, 0x00, // angry eye x
  0x02, 0x01, 0x58, 0xA4, 0x62, 0x1C, 0x00, 0x00,
};

//wheel-left
byte wheel_left2[][8] =
{
  0x00, 0x40, 0x9A, 0x25, 0x46, 0x38, 0x00, 0x00, // angry eye y
  0x40, 0x80, 0x1A, 0x25, 0x46, 0x38, 0x00, 0x00,
};

//wheel-right
byte wheel_right1[][8] =
{
  0x01, 0x03, 0x01, 0x00, 0x5C, 0xA2, 0x62, 0x00,
  0x02, 0x07, 0x03, 0x01, 0x5C, 0xA2, 0x62, 0x00, // heart eyes x

};

//wheel-right
byte wheel_right2[][8] =
{
  0x40, 0xE0, 0xC0, 0x80, 0x3A, 0x45, 0x46, 0x00,
  0x80, 0xC0, 0x80 ,0x00, 0x3A, 0x45, 0x46, 0x00, // heart eyes y

};

//static emotion -left
byte static_emotion_left[][8] =
{
  
};

//static emotion -right
byte static_emotion_right[][8] =
{
  
};

//clear
byte clearEmotion[][8] = {
  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};

Freenove_ESP32_VK16K33 matrix = Freenove_ESP32_VK16K33();
int emotion_time_next = 0;
int emotion_time_now = 0;
int emotion_count = 0;

//rotate eyes
void eyesRotate(int delay_ms)
{
  int count = sizeof(eyeRotate1) / sizeof(eyeRotate1[0]);
  emotion_time_next = millis();
  if (emotion_time_next - emotion_time_now >= delay_ms)
  {
    emotion_time_now = emotion_time_next;
    matrix.showStaticArray(eyeRotate1[emotion_count], eyeRotate2[emotion_count]);
    emotion_count++;
    if (emotion_count >= count)
      emotion_count = 0;
  }
}

//Blink eyes
void eyesBlink(int delay_ms)
{
  emotion_time_next = millis();
  if (emotion_time_next - emotion_time_now >= delay_ms)
  {
    emotion_time_now = emotion_time_next;
    emotion_count++;
    if (emotion_count <= 10)
      matrix.showStaticArray(eyeBlink1[0], eyeBlink2[0]);
    else if (emotion_count > 10)
    {
      int count = sizeof(eyeBlink1) / sizeof(eyeBlink1[0]);
      matrix.showStaticArray(eyeBlink1[(emotion_count - 11)], eyeBlink2[(emotion_count - 11)]);
      if ((emotion_count - 10) >= count)
        emotion_count = 0;
    }
  }
}

//smile
void eyesSmile(int delay_ms)
{
  int count = sizeof(eyeSmile1) / sizeof(eyeSmile1[0]);
  emotion_time_next = millis();
  if (emotion_time_next - emotion_time_now >= delay_ms)
  {
    emotion_time_now = emotion_time_next;
    matrix.showStaticArray(eyeSmile1[emotion_count], eyeSmile2[emotion_count]);
    emotion_count++;
    if (emotion_count >= count)
      emotion_count = 0;
  }
}

//cry
void eyesCry(int delay_ms)
{
  int count = sizeof(eyeCry1) / sizeof(eyeCry1[0]);
  emotion_time_next = millis();
  if (emotion_time_next - emotion_time_now >= delay_ms)
  {
    emotion_time_now = emotion_time_next;
    matrix.showStaticArray(eyeCry1[emotion_count], eyeCry2[emotion_count]);
    emotion_count++;
    if (emotion_count >= count)
      emotion_count = 0;
  }
}

//wheel
void wheel(int mode, int delay_ms)
{
  if (mode == 1)
  {
    int count = sizeof(wheel_left1) / sizeof(wheel_left1[0]);
    emotion_time_next = millis();
    if (emotion_time_next - emotion_time_now >= delay_ms)
    {
      emotion_time_now = emotion_time_next;
      matrix.showStaticArray(wheel_left1[emotion_count], wheel_left2[emotion_count]);
      emotion_count++;
      if (emotion_count >= count)
        emotion_count = 0;
    }
  }
  if (mode == 2)
  {
    int count = sizeof(wheel_right1) / sizeof(wheel_right1[0]);
    emotion_time_next = millis();
    if (emotion_time_next - emotion_time_now >= delay_ms)
    {
      emotion_time_now = emotion_time_next;
      matrix.showStaticArray(wheel_right1[emotion_count], wheel_right2[emotion_count]);
      emotion_count++;
      if (emotion_count >= count)
        emotion_count = 0;
    }
  }
}

//show static emotion
void staticEmtions(int emotion)
{
  int count = sizeof(static_emotion_left) / sizeof(static_emotion_left[0]);
  emotion = constrain(emotion, 0, count-1);
  matrix.showStaticArray(static_emotion_left[emotion], static_emotion_right[emotion]);
}

//clear all
void clearEmtions(void)
{
  emotion_count = 0;
  int count = sizeof(clearEmotion) / sizeof(clearEmotion[0]);
  for (int i = 0; i < count; i++)
  {
    matrix.showStaticArray(clearEmotion[i], clearEmotion[i]);
  }
}

//initialize HT16K33
void Emotion_Setup()
{
  matrix.init(EMOTION_ADDRESS, EMOTION_SDA, EMOTION_SCL);
  matrix.setBrightness(15);
  emotion_time_now = millis();
}

int emotion_task_mode = 0;
//Emotion show
void Emotion_Show(int mode)
{
  if (mode == 0)
    clearEmtions();
  else if (mode == 1)
    eyesRotate(150);   
  else if (mode == 2)
    eyesCry(200); 
  else if (mode == 3)
    eyesSmile(200);                  
  else if (mode == 4)
    wheel(2, 100);
  else if (mode == 5)
    wheel(1, 100);
  else if (mode == 6)
    eyesBlink(100);    
}

//Emotion set mode
void Emotion_SetMode(int mode)
{
  if (mode <= 6)
    emotion_task_mode = mode;
  else if (mode >= 7)
  {
    emotion_task_mode = 7;
    int emotion_flag = millis() % 22;
    Serial.println("emotion_flag:"+String(emotion_flag));
    staticEmtions(emotion_flag);
  }
}